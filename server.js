...
app.post('/tabs/:tabId/download', express.json({ limit: '16kb' }), async (req, res) => {
  try {
    ...
    const requestContext = found.tabState.page.context().request;
    let downloadUrl = requestedUrl;
    let response;
    for (let redirects = 0; redirects <= 10; redirects++) {
      response = await requestContext.get(downloadUrl.href, {
        maxRedirects: 0,
        timeout: 90_000,
      });
      ...
      if (!location) {
        return res.status(502).json({ error: 'Resource redirect was missing a Location header' });
      }

      const nextUrl = new URL(location, downloadUrl);
      if (nextUrl.origin !== requestedUrl.origin) {
        return res.status(400).json({ error: 'Resource redirects must remain on the tab origin' });
      }
      if (redirects === 10) {
        return res.status(400).json({ error: 'Resource exceeded the redirect limit' });
      }
      downloadUrl = nextUrl;
    }
    ...
    const fallback = downloadUrl.pathname.split('/').pop() || 'download.bin';
